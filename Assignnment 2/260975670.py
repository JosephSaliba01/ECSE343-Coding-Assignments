...
def EM(gmm):
    ...
    for n in range(1, gmm.max_iter):
        ...
        if gmm.verbose:
            print('Iteration: {:4d} - log likelihood: {:1.6f}'.format(n,
                  gmm.log_likelihoods[-1]), flush=True)
...
def PolRegMLE(p, mu, x, y):
    ...
    for i in range(0, A.shape[0]):
        for j in range(0, A.shape[1]):
            A[i, j] = sum(x[s] ** (i + j) for s in range(0, n))

    for i in range(0, b.shape[0]):
        b[i] = sum(x[s] ** i * (y[s] - mu) for s in range(0, n))
...
def PolRegMAP(p, mu_noise, sigma_noise, mu_params, sigma_params, x, y):
    ...
    for i in range(0, A.shape[0]):
        for j in range(0, A.shape[1]):
            if i == j:
                A[i, j] = sum(x[s]**(i+j) for s in range(0, n)) + \
                    (sigma_noise / sigma_params)**2
            else:
                A[i, j] = sum(x[s] ** (i+j) for s in range(0, n))

    for i in range(0, b.shape[0]):
        b[i] = sum(x[s] ** i * (y[s] - mu_noise) for s in range(0, n)
                   ) + mu_params * (sigma_noise / sigma_params) ** 2
    return A, b
...
def PolRegLLS(p, n, x, y):
    ...
    for i in range(0, b.shape[0]):
        b[i] = sum(x[s] ** i * y[s] for s in range(0, n))
...
class GMM:
    def __init__(self, X, n_components=10, reg_covar=1e-2, tol=1e-4,
        ...
        self.reg_cov = self.reg_covar * \
            np.identity(self.n_dim, dtype=np.float32)

        # initial (isotropic) covariance extent
        self.init_covar = 0.5 * (np.amax(X) - np.amin(X)) / self.n_components

        # initial covariance matrix
        self.init_cov = self.init_covar * \
            np.identity(self.n_dim, dtype=np.float32)

        # Initialize the mu, covariance and pi values
        if mu_init is None:
            # Initialize mean vector as random element of X
            self.mu = self.X[np.random.choice(
                range(0, self.n_samples), self.n_components, replace=False), :]
        else:
            try:
                assert(
                    mu_init.shape[0] == self.n_components and mu_init.shape[1] == self.n_dim)
            except:
        ...
        self.cov = np.zeros((self.n_components, self.n_dim,
                            self.n_dim), dtype=np.float32)
        ...
        self.pi = np.full(self.n_components, 1. /
        ...
        # Initialized to 0
        self.alpha = np.zeros(
            (self.n_samples, self.n_components), dtype=np.float32)
    ...
    def draw_ellipse(self, k, **kwargs):
        ...
        for nsig in range(1, 4):
            ax.add_patch(
                Ellipse(self.mu[k, :], nsig * width, nsig * height, angle, **kwargs))
    ...
    def plotGMM(self, samples=None, labels=None, ellipse=True):
        ...
        if samples is None or labels is None:
            ...
            try:
                assert(
                    self.n_dim == samples.shape[1] and samples.shape[0] == labels.shape[0])
            except:
                raise Exception('Can\'t plot if not 2D')
            plt.scatter(samples[:, 0], samples[:, 1],
                        c=colors[labels, :], s=10)
        plt.axis('equal')
        plt.axis('tight')
        ...
        if ellipse:
            for k in range(self.n_components):
                self.draw_ellipse(k, alpha=w_factor *
                                  self.weight[k], color=colors[k, :3])
...
def main():
    ...
    A_MAP, b_MAP = PolRegMAP(p, mu_noise, sigma_noise,
                             mu_params, sigma_params, x, y)
    ...
    axes.bar(np.arange(p) + 0.125, theta_MLE, 0.125,
             label=r"$\theta_{MLE}$ error: " + str(round(np.linalg.norm(theta_MLE-theta, 2), 2))),
    axes.bar(np.arange(p) + 0.250, theta_MAP, 0.125,
             label=r"$\theta_{MAP}$ error: " + str(round(np.linalg.norm(theta_MAP-theta, 2), 2))),
    axes.bar(np.arange(p) + 0.375, theta_LLS, 0.125,
             label=r"$\theta_{LLS}$ error: " + str(round(np.linalg.norm(theta_LLS-theta, 2), 2)))
    axes.legend()
    # This is a blocking call! Close the plotting window to continue execution flow.
    plt.show()
    ...
    plt.plot(z, np.polyval(theta_MLE[::-1], z), '--',
             label='$p_{{ {0} }} (x | \\theta_{{MLE}})$'.format(m))
    plt.plot(z, np.polyval(theta_MAP[::-1], z), '--',
             label='$p_{{ {0} }} (x | \\theta_{{MAP}})$'.format(m))
    plt.plot(z, np.polyval(theta_LLS[::-1], z), '--',
             label='$p_{{ {0} }} (x | \\theta_{{LLS}})$'.format(m))
    plt.legend()
    # This is a blocking call! Close the plotting window to continue execution flow.
    plt.show()
